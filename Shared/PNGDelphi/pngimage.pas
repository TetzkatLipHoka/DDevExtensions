...
{$IFNDEF DDEV_PNG_NOINIT}        //DDevExtensions: -DDDEV_PNG_NOINIT builds the unit
                                 //without initialization-time self-registration; the
                                 //host (FixAlphaControlsPNG) registers on demand
...
{$DEFINE PNGGraphic}             //Fix AlphaControls acPNG
{.$DEFINE PNGGraphicBMP}          //Fix AlphaControls acPNG (Convert to Bitmap)
{$RANGECHECKS OFF} {$J+}
...
type
  ...
  TPngDPIMode = ( dmGlobal, dmIgnore, dmUse );
  ...
  protected
    ...
    function GetSupportsPartialTransparency: Boolean; {$IF CompilerVersion >= 20}override;{$IFEND}
  ...
  public
    ...
    class function CanLoadFromStream(Stream: TStream): Boolean; {$IF CompilerVersion >= 32}override;{$IFEND}
    ...
    property DPIMode: TPngDPIMode read fDPIMode write fDPIMode;
  end;

  TPNGObject = TPngImage; // deprecated 'Use TPngImage.';

  {$IFDEF PNGGraphic}
  TPNGGraphic = class( TPngImage ) // Converter for AlphaControls (acPNG)
  protected
  ...
  {$IFDEF PNGGraphicBMP}
  TPNGGraphic = class( TBitmap ) // Converter for AlphaControls (acPNG)
  protected
    procedure WriteData(Stream: TStream); override;
  public
    procedure LoadFromStream(Stream: TStream); override;
...
{$ALIGN ON}
  ...
  private
    ...
    procedure SetDPIX( Value : Cardinal );
    function GetDPIY : Cardinal;
    procedure SetDPIY( Value : Cardinal );
  public
...
const
  PixelPerMeterToPixelPerInchFactor = 39.3701;
  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        next_out := Pointer(PAnsiChar(Output) + total_out);
        avail_out := 256;
      end {if (avail_out = 0)};
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {Copies the new data}
        CopyMemory(PAnsiChar(Output) + OutputSize - total_out,
          @Buffer, total_out);
...
begin
  ...
  {Reads data}
  fYear := ((pByte(UIntPtr(Data) )^) * 256)+ (pByte(PAnsiChar(Data) + 1)^);
  fMonth := pByte(PAnsiChar(Data) + 2)^;
  fDay := pByte(PAnsiChar(Data) + 3)^;
  fHour := pByte(PAnsiChar(Data) + 4)^;
  fMinute := pByte(PAnsiChar(Data) + 5)^;
  fSecond := pByte(PAnsiChar(Data) + 6)^;
end;
...
begin
  ...
  pByte(PAnsiChar(Data) + 2)^ := Month;
  pByte(PAnsiChar(Data) + 3)^ := Day;
  pByte(PAnsiChar(Data) + 4)^ := Hour;
  pByte(PAnsiChar(Data) + 5)^ := Minute;
  pByte(PAnsiChar(Data) + 6)^ := Second;
...
begin
  ...
  if UIntPtr(fKeyword) = 0 then
    ...
    CompressionMethod := PByte(PAnsiChar(fKeyword) + Length(fKeyword))^;
  fText := '';
  ...
  begin
    Output := nil;
    if DecompressZLIB(PAnsiChar(PAnsiChar(Data) + Length(fKeyword) + 2),
...
begin
  ...
  begin
    ...
    PByte(PAnsiChar(Data) + Length(Keyword) + 1)^ := 0;
    if OutputSize > 0 then
      CopyMemory(PAnsiChar(Data) + Length(Keyword) + 2, Output, OutputSize);
...
begin
  ...
  CopyMemory(@fText[1], PAnsiChar(Data) + Length(fKeyword) + 1, Length(fText));
end;
...
begin
  ...
  if Keyword <> '' then
    CopyMemory(Data, @fKeyword[1], Length(Keyword));
  if Text <> '' then
    CopyMemory(PAnsiChar(Data) + Length(Keyword) + 1, @fText[1],
      Length(Text));
  {Let ancestor calculate crc and save}
  Result := inherited SaveToStream(Stream);
end;
...
begin
  ...
  begin
    ...
    {Copy palette also}
//    CopyPalette(TChunkIHDR(Source).ImagePalette, ImagePalette); // MS
...
begin
  ...
  Dest := PByte( PAnsiChar( Dest ) + Col * 3 );
  repeat
    {Copy this row}
    Dest^ := fOwner.GammaTable[PByte(PAnsiChar( Src ) + 2)^]; inc(Dest);
    Dest^ := fOwner.GammaTable[PByte(PAnsiChar( Src ) + 1)^]; inc(Dest);
    Dest^ := fOwner.GammaTable[PByte(PAnsiChar( Src )   )^]; inc(Dest);
...
begin
  ...
  repeat
    {Copy this row}
    Dest^ := Owner.GammaTable[PByte( PAnsiChar(Src ) + 4)^]; inc(Dest);
    Dest^ := Owner.GammaTable[PByte( PAnsiChar(Src ) + 2)^]; inc(Dest);
    Dest^ := Owner.GammaTable[PByte( PAnsiChar(Src )    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
    Extra^ := fOwner.GammaTable[PByte( PAnsiChar(Src ) + 5)^]; inc(Extra);
    Extra^ := fOwner.GammaTable[PByte( PAnsiChar(Src ) + 3)^]; inc(Extra);
    Extra^ := fOwner.GammaTable[PByte( PAnsiChar(Src ) + 1)^]; inc(Extra);
    {$ENDIF}
...
begin
  ...
  repeat
    ...
    repeat
      ...
      Dest2 := PByte( PAnsiChar( Dest ) + (Header.BitDepth * Col) div 8 );
      {Copy data}
...
begin
  ...
  repeat
    ...
    repeat
      {Adjust pointer to pixel byte bounds}
      Dest2 := PByte( PAnsiChar( Dest ) + Col div 2 );
      {Copy data}
...
begin
  {Get first column and enter in loop}
  Col := ColumnStart[Pass];
  Dest := PByte( PAnsiChar( Dest ) + Col );
  repeat
    {Copy this row}
    Dest^ := Src^; inc(Dest);
    {$IFDEF Store16bits}
    Extra^ := PByte(PByte( PAnsiChar( Src ) + 1 ))^; inc(Extra);
    {$ENDIF}
...
begin
  ...
  Col := ColumnStart[Pass];
  Dest := PByte( PAnsiChar( Dest ) + Col * 3 );
  Trans := PByte( PAnsiChar( Trans ) + Col );
  repeat
    {Copy this row and alpha value}
    Trans^ := PByte( PAnsiChar( Src ) + 3)^;
    Dest^ := fOwner.GammaTable[PByte( PAnsiChar( Src ) + 2)^]; inc(Dest);
    Dest^ := fOwner.GammaTable[PByte( PAnsiChar( Src ) + 1)^]; inc(Dest);
    Dest^ := fOwner.GammaTable[PByte( PAnsiChar( Src )    )^]; inc(Dest);
...
begin
  ...
  repeat
    {Copy this row and alpha value}
    Trans^ := PByte( PAnsiChar( Src ) + 6)^;
    PByte(Dest)^  := fOwner.GammaTable[PByte( PAnsiChar( Src ) + 4)^]; inc(Dest);
    PByte(Dest)^  := fOwner.GammaTable[PByte( PAnsiChar( Src ) + 2)^]; inc(Dest);
    PByte(Dest)^  := fOwner.GammaTable[PByte( PAnsiChar( Src )    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
    Extra^ := fOwner.GammaTable[PByte( PAnsiChar( Src ) + 5)^]; inc(Extra);
    Extra^ := fOwner.GammaTable[PByte( PAnsiChar( Src ) + 3)^]; inc(Extra);
...
begin
  ...
  repeat
    {$IFDEF Store16bits}
    Extra^ := PByte( PAnsiChar( Src ) + 1)^; inc(Extra);
    {$ENDIF}
...
begin
  ...
  begin
    ...
    Data := PByte( PAnsiChar(Header.ImageData) + Header.BytesPerRow *
      (ImageHeight - 1 - CurrentRow) );
    Trans := PByte( PAnsiChar(Header.ImageAlpha) + ImageWidth * CurrentRow );
    {$IFDEF Store16bits}
    Extra := PByte( PAnsiChar(Header.ExtraImageData) + Header.BytesPerRow *
      (ImageHeight - 1 - CurrentRow) );
    {$ENDIF}
...
begin
  ...
  begin
    ...
    {Copy pixel values}
    Dest^ := fOwner.GammaTable[PByte( PAnsiChar( Src ) + 4)^]; inc(Dest);
    Dest^ := fOwner.GammaTable[PByte( PAnsiChar( Src ) + 2)^]; inc(Dest);
    Dest^ := fOwner.GammaTable[PByte( PAnsiChar( Src )    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
...
begin
  ...
  Data := PByte( PAnsiChar(Header.ImageData) + Header.BytesPerRow * (ImageHeight - 1) );
  Trans := Header.ImageAlpha;
  {$IFDEF Store16bits}
  Extra := PByte( PAnsiChar(Header.ExtraImageData) + Header.BytesPerRow * (ImageHeight - 1) );
  {$ENDIF}
  {Reads each line}
...
begin
  ...
  begin
    {Copy pixel values}
    Dest^ := fOwner.InverseGamma[PByte( PAnsiChar( Src ) + 2)^]; inc(Dest);
    Dest^ := fOwner.InverseGamma[PByte( PAnsiChar( Src ) + 1)^]; inc(Dest);
    Dest^ := fOwner.InverseGamma[PByte( PAnsiChar( Src )    )^]; inc(Dest);
    {Move to next pixel}
    inc(Src, 3);
  end {for I}
end;
...
begin
  ...
  begin
    ...
    //for sample
    {Copy pixel values}
    pWORD(Dest)^ := fOwner.InverseGamma[PByte( PAnsiChar( Src ) + 2)^]; inc(Dest, 2);
    pWORD(Dest)^ := fOwner.InverseGamma[PByte( PAnsiChar( Src ) + 1)^]; inc(Dest, 2);
    pWORD(Dest)^ := fOwner.InverseGamma[PByte( PAnsiChar( Src )    )^]; inc(Dest, 2);
    {Move to next pixel}
    inc(Src, 3);
  end {for I}
...
begin
  ...
  begin
    //Now we copy from 1 byte for each sample stored to a 2 bytes (or 1 word)
    //for sample
    pWORD(Dest)^ := PByte( PAnsiChar( Src ))^; inc(Dest, 2);
    {Move to next pixel}
    inc(Src);
  end {for I}
end;
...
begin
  ...
  FOR i := 1 TO ImageWidth do
  begin
    Dest^ := Owner.InverseGamma[PByte( PAnsiChar( Src ) + 2)^]; inc(Dest);
    Dest^ := Owner.InverseGamma[PByte( PAnsiChar( Src ) + 1)^]; inc(Dest);
    Dest^ := Owner.InverseGamma[PByte( PAnsiChar( Src )    )^]; inc(Dest);
    Dest^ := Trans^; inc(Dest);
    inc(Src, 3); inc(Trans);
  end {for i};
end;
...
begin
  ...
  FOR i := 1 TO ImageWidth do
  begin
    pWord(Dest)^ := Owner.InverseGamma[PByte( PAnsiChar( Src ) + 2)^]; inc(Dest, 2);
    pWord(Dest)^ := Owner.InverseGamma[PByte( PAnsiChar( Src ) + 1)^]; inc(Dest, 2);
    pWord(Dest)^ := Owner.InverseGamma[PByte( PAnsiChar( Src )    )^]; inc(Dest, 2);
...
begin
  ...
  with Header.BitmapInfo.bmiHeader do
    repeat
      ...
      repeat
        {Adjust pointer to pixel byte bounds}
        Src2 := PByte( PAnsiChar( Src ) + (biBitCount * Col) div 8 );
        {Copy data}
        Dest^ := Byte(Dest^) or
...
begin
  ...
  PByte(ImageSource) := PByte( PAnsiChar(Header.ImageData) +
  ...
  case Header.BitmapInfo.bmiHeader.biBitCount of
    ...
    24:
      ...
      begin
        ...
        PByte(ImageSource) := PByte( PAnsiChar(ImageSourceOrg) - BytesPerRowSrc * j2 );
        PByte(AlphaSource) := PByte( PAnsiChar(Header.ImageAlpha) + BytesPerRowAlpha * j2 );
      end;
    ...
    1,4,8: if Header.ColorType = COLOR_GRAYSCALEALPHA then
      ...
      begin
        ...
        {Move pointers}
        PByte(ImageData) := PByte( PAnsiChar(ImageData) + BytesPerRowDest );
        if Stretch then j2 := trunc(j / FactorY) else j2 := j;
        PByte(ImageSource) := PByte( PAnsiChar(ImageSourceOrg) - BytesPerRowSrc * j2 );
        PByte(AlphaSource) := PByte( PAnsiChar(Header.ImageAlpha) + BytesPerRowAlpha * j2 );
      end
...
begin
  with Header do
    if (ColorType = COLOR_RGBALPHA) or (ColorType = COLOR_GRAYSCALEALPHA) then
      PByte(Result) := PByte( PAnsiChar(ImageAlpha) + (Cardinal(LineIndex) * Width) )
...
begin
  with Header do
    PByte(Result) := PByte( PAnsiChar(ExtraImageData) + ((Height - 1) *
...
begin
  with Header do
    PByte(Result) := PByte( PAnsiChar(ImageData) + ((Height - 1) *
      Cardinal(BytesPerRow)) - (LineIndex * BytesPerRow) );
end;
...
begin
  ...
  begin
    ...
    for Line := 0 to Min(CY - 1, Height - 1) do
      CopyMemory( PByte( PAnsiChar(NewImageData) + (CY - 1) *
    ...
    begin
      ...
      for Line := 0 to Min(CY - 1, Height - 1) do
        CopyMemory(PByte( PAnsiChar(NewImageAlpha) + (Line * CX)),
    ...
    begin
      ...
      for Line := 0 to Min(CY - 1, Height - 1) do
        CopyMemory(PByte( PAnsiChar(NewImageExtra) + (Line * CX)),
...
function TChunkpHYs.GetDPIX : Cardinal;
begin
  if ( fUnit <> utMeter ) then
    result := 0
  else
    result := Round( fPPUnitX / PixelPerMeterToPixelPerInchFactor );
end;

procedure TChunkpHYs.SetDPIX( Value : Cardinal );
begin
  if ( fUnit <> utMeter ) then
    Exit;

  fPPUnitX := Round( Value * PixelPerMeterToPixelPerInchFactor );
end;

function TChunkpHYs.GetDPIY : Cardinal;
begin
  if ( fUnit <> utMeter ) then
    result := 0
  else
    result := Round( fPPUnitY / PixelPerMeterToPixelPerInchFactor );
end;

procedure TChunkpHYs.SetDPIY( Value : Cardinal );
begin
  if ( fUnit <> utMeter ) then
    Exit;

  fPPUnitY := Round( Value * PixelPerMeterToPixelPerInchFactor );
end;
...
begin
  ...
  {Reads data}
  fPPUnitX := ByteSwap(pCardinal(Data)^);
  fPPUnitY := ByteSwap(pCardinal(PAnsiChar(Data) + 4)^);
  fUnit := pUnitType(PAnsiChar(Data) + 8)^;

  if ( ( ( Owner.DPIMode = dmGlobal ) AND IgnoreImageDPI ) OR ( Owner.DPIMode = dmIgnore ) ) then
    begin
    DC := GetDC(0);
...
begin
  ...
  pCardinal(PAnsiChar(Data) + 4)^ := ByteSwap(fPPUnitY);
  pUnitType(PAnsiChar(Data) + 8)^ := fUnit;
...
begin
  ...
  {Test if the header matches}
  if Header = ClassName then // PngHeaderAC then
    begin
    found := False;
    for i := 0 to 10 do
      begin
...
initialization
  ...
  {$IFDEF UseDelphi}{$IFDEF RegisterGraphic}
    {$IF Defined( PNGGraphic ) OR Defined( PNGGraphicBMP )}
